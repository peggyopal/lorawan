  ...
  for (NodeContainer::Iterator j = endDevices.Begin (); j != endDevices.End (); ++j)
    {
      ...
      // NS_LOG_DEBUG (loraNetDevice->GetMac ()->GetObject<LorawanMacHelper> ());
      Ptr<EndDeviceLorawanMac> mac = loraNetDevice->GetMac ()->GetObject<EndDeviceLorawanMac> ();
      // Ptr<ClassCEndDeviceLorawanMac> mac = loraNetDevice->GetMac ()->GetObject<ClassCEndDeviceLorawanMac> ();
      NS_ASSERT (mac != 0);
